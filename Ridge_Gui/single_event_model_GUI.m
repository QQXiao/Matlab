function out = single_event_model_GUI(featdir,write_std_image,save_lambda,use_raw)
    ...
    % 2010/16/4  Jeanette Mumford:  Adapted this to Gui's design so
    %            the last EV is not broken up by trial
 ...
 %  addpath /space/raid/fmri/NIFTI_20090909/
    ...
    for evnum=1:(design.evs_orig-1)   %Only the last EV is omitted JM4/16/10
   ...
   %JM 4/16/2010:  Adding the last regressor.  Note I'm grabbing it
   % from the design.mat file, so I'm assuming HP filtering has already
   % been applied

   %read in design.mat
   fsl_desmat=read_design_mat([featdir,'/design.mat']);
   %I AM ASSUMING THE DERIVATIVE WAS ENTERED INTO THE MODEL!!
 
   x_single_hp=[X_single_hp, fsl_desmat(:,(end-1))];
