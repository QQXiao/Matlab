function out = single_event_model(featdir)
    % Usage: single_event_model(featdir)
    ...
    if exist('filtered_func_data.nii')+exist('filtered_func_data.nii.gz')>0
      filt_file=dir('filtered_func_data.*');
      filt_file=filt_file.name;
    else
      filt_file=dir('filtered_func_data.img*');
      filt_file=filt_file.name;
    end


    dataf = strcat(featdir, '/', filt_file);
   ...
   X_single_hp=F*X_single;
    ...
    fname_beta_est = strcat(ridge_path, '/ridge_beta_white_lam_color.nii');
    ...
    system(sprintf(['avworient -forceradiological %s'], fname_beta_est));
    ...
    system(sprintf(['avworient -forceradiological %s'], fname_lambda));
    time2=toc;
    fprintf(' %f sec\n \n', time2);
    
    %Standardization
    fprintf('Standardization...');
    std_beta_est = strcat(ridge_path, '/ridge_beta_white_lam_color_std');
    std_lambda = strcat(ridge_path, '/lambda_color_std');
    std_beta_ls_est = strcat(ridge_path, '/ls_beta_white_std');
    mat_file = strcat(featdir, '/reg/example_func2standard.mat');
    time3 = 0;
    tic
    system(sprintf(['applyxfm4D %s /space/raid/fmri/fsl/etc/standard/avg152T1.img %s %s -singlematrix'], fname_beta_est, std_beta_est, mat_file));

    system(sprintf(['flirt -in %s -ref /space/raid/fmri/fsl/etc/standard/avg152T1.img -out %s -applyxfm  -init %s'], fname_lambda, std_lambda, mat_file));
    time3 = toc;
    fprintf(' %f sec\n \n', time3);
