function out = single_event_model(featdir,std_ref,MkStdImages)
    ...
    %load onsets
    ons=[];
    for c=1:design.evs_orig,
    	onsetf = eval(sprintf('design.custom%d',c));
...
%    system(sprintf(['applyxfm4d.sh %s %s %s'],fname_beta_est, std_ref, mat_file));
    system(sprintf('mv ridge_beta_std.nii.gz %s',ridge_path));
