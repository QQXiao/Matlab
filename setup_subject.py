...
def parse_command_line():
    ...
    parser.add_argument('--skip', dest='skip', action='store_true',
    	default=False, help='skip over scans with unreadable dicom headers')
...
def convert_dicom_to_nifti(args, subdir):
	...
	for f in boldfiles:
		runnum=f.rsplit('a')[-2].rsplit('s')[-1].lstrip('0')
                runprefix = dcmhdrs[runnum].ProtocolName
                if runprefix == "":
                        runprefix = dcmhdrs[runnum].SeriesDescription
		runname=runprefix.replace(' ','_')
